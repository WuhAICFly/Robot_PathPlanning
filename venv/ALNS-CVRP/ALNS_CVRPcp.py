...
def scanning_method(distance,locations,demands):
    ...
    x = temp_loca[:,0].astype(float)
    y = temp_loca[:,1].astype(float)
    ...
    temp_list = concat[:,1].astype(int).tolist()
...
def readtxt(line):
    with open('data.txt', 'r') as f:
        lines = f.readlines()
        Customer = lines[line]  # 获取第1行的字符串，下标从0开始
        Demand = lines[line+1]  # 获取第2行的字符串，下标从0开始
        print(Customer,Demand)
    my_string1 = Customer
    my_string2 = Demand
    result1 = my_string1.split('=')[1]
    result2 = my_string2.split('=')[1]
    customer = eval(result1)
    demand = eval(result2)
    print(len(customer))
    print(len(demand))
    return customer,demand
def writetxt_savefig(i,file):

        # 将结果写入文件
     Bestcost = str(best_cost)
     result=str(i//2+1) + " " + Bestcost + "\n"
     file.write(result)
     figname = str(i//2+1)
