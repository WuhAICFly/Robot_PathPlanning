...
def tspPos(indd,flag,currentPath,ppos,pos,i,path,lst,idnum,reorder_p,new_lst,tpos):
    ...
    if goal != []:
        ...
        curroad = newcurroad
        if newcurroad != []:
          currentPath=findcurrentPath(newcurroad)
    else:
...
def findcurrentPath(newcurroad):
    lst=[]
    ...
    for p in newcurroad:
        for pp in p:
          lst.append(pp[0])
    print("findlst:",lst)
    list = [item for item in lst if item != lst[0]]
    print(list)
    nearest_neighbors, ind1 = find_k_nearest_neighbors(list, lst1[0], 2)
    print("离出发点最近nearest_neighbors:", nearest_neighbors)
    k_neighbors = [x[0] for x in nearest_neighbors]
    print(k_neighbors)
    # 求最近邻所在路段索引
    id = findindex(k_neighbors[0], newcurroad)
    print(id)
    print("路点在路段pos[id]", newcurroad[id])  # 确定拆分起始路段
    currentPath = newcurroad[id]
    return currentPath
